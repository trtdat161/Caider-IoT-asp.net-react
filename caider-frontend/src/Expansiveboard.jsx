...
export function Expansiveboard() {
  ...
  const data = async (page = 1) => {
    try {
      ...
      setExpansive(result.items || []);
      setTotalPages(Math.ceil(result.cntTotal / result.pageSize));
      setPageNumber(result.pageNumber);
    } catch (error) {
      console.error("Lỗi khi lấy dữ liệu:", error);
      setLoadError("Lỗi khi lấy dữ liệu expansive board: " + error.message);
      setExpansive([]);
    }
  };
  ...
  const deletes = async (id) => {
    try {
      ...
      setDeleteError("Lỗi khi xóa dữ liệu expansive board: " + error.message);
    }
  };
  ...
  return (
    <>
      ...
      <div className="d-flex justify-content-between align-items-center mb-5">
        <h1 className="text-light ms-2 fw-bold">Dashboard Expansive Board</h1>
        <button
          className="m-2 btn btn-primary fw-medium"
          onClick={() => navigate("/manage/add-or-up-expansive")}
        >
      ...
      <div className="mb-4 d-flex justify-content-end align-items-center me-2">
        <div className="position-relative">
          <input
            ...
            placeholder="Search expansive board..."
        ...
        <table className="table table-hover align-middle mb-0 ">
          <thead className="table-dark">
            <tr>
              ...
              <th className="text-center fw-bold">expansive board</th>
          ...
          <tbody>
            {filterExpansive.map((item, index) => {
              return (
                <>
                  <tr key={item.id}>
                    ...
                    <td>
                      <button
                        ...
                        onClick={() =>
                          navigate(`/manage/add-or-up-expansive/${item.id}`)
                        }
                      >
                        Edit
                      </button>
